...
Param (
    ...
    [ValidateSet("BroCi", "AuthCode", "DeviceCode", "ManualCode", "BroCiManualCode", "BroCiToken", "ClientCredentials")]
    ...
    # App-only (ClientCredentials) parameters
    [Parameter(Mandatory = $false)]
    [string]$ClientId,
...
if ($AuthFlow -ne "BroCiToken" -and $AuthFlow -ne "ClientCredentials" -and -not [string]::IsNullOrWhiteSpace($BroCiToken)) {
...
# Check non-Windows auth flow compatibility (Linux/macOS) — skip for ClientCredentials (always supported)
...
# Resolve ClientCredentials params (client secret from env var EF_CLIENT_SECRET)
if ($AuthFlow -eq "ClientCredentials") {
    if ([string]::IsNullOrWhiteSpace($ClientId)) {
        Write-Error "ClientCredentials vereist -ClientId." -ErrorAction Stop
    }
    $efClientSecret = $env:EF_CLIENT_SECRET
    if ([string]::IsNullOrWhiteSpace($efClientSecret)) {
        Write-Error "ClientCredentials vereist de omgevingsvariabele EF_CLIENT_SECRET." -ErrorAction Stop
    }
    if ([string]::IsNullOrWhiteSpace($AppTenantId) -and -not [string]::IsNullOrWhiteSpace($Tenant)) {
        $AppTenantId = $Tenant
    }
    if ([string]::IsNullOrWhiteSpace($AppTenantId)) {
        Write-Error "ClientCredentials vereist -AppTenantId of -Tenant." -ErrorAction Stop
    }
}
...
if ($AuthFlow -eq "ClientCredentials") {
    $GLOBALAuthMethods.ClientId     = $ClientId
    $GLOBALAuthMethods.ClientSecret = $efClientSecret
    $GLOBALAuthMethods.AppTenantId  = $AppTenantId
}
...
} elseif ($AuthFlow -eq "ClientCredentials") {
    Write-Log -Level Verbose -Message "[SecurityFindings] ClientCredentials flow: reusing main Graph token for security findings."
